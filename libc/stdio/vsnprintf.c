...
static int _vsnprintf_engine(out_fct_type out, char* buffer, size_t maxlen, const char* format, va_list va) {
...
int vsnprintf(char* buffer, size_t maxlen, const char* format, va_list va) {
  int ret = _vsnprintf_engine(_out_buffer, buffer, maxlen, format, va);
  if (buffer && maxlen > 0) {
    buffer[ret < (int)maxlen ? ret : (int)maxlen - 1] = '\0';
  }
  return ret;
}
...
int vsprintf(char* buffer, const char* format, va_list va) {
  int ret = _vsnprintf_engine(_out_buffer, buffer, (size_t)-1, format, va);
  if (buffer) {
    buffer[ret] = '\0';
  }
  return ret;
